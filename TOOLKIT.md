...
> **Looking for a different AI agent?** This repo has separate branches for each supported tool. Switch to the branch matching your agent — `Claude Code`, `cursor`, `claude-code`, `opencode`, or `antigravity` — to get the right configuration and code.
...
2. **Open Claude Code Chat** in your editor.
3. **Type a slash command** like `/hint`, `/debug`, `/explain`, etc.
4. **Answer the questions** Claude Code asks you. Don't skip them — that's where the learning happens.
5. **Write the code yourself.** AI will guide, not write.

> **Important!:** Disable Claude Code inline completions while learning. Use Chat mode only.

---

## The 12 Prompts

Each prompt is a workflow. Invoke it with a `/command` in Claude Code.
...
- **Claude Code** (Chat mode) in your editor
...
- **Skills** require a Claude Code plan that supports custom skills. Prompts work everywhere.
...
- This is a prompt-based toolkit, not an app. It works through Claude Code conversations.
