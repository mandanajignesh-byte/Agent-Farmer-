...
# market.py inlined - the module was deleted when it was folded into main.py,
# and this snapshot still imported it, so it scored its $3,000 starting money
# in silence. Names are prefixed to avoid colliding with the agent's PARAMS.
"""Model of the market price curve.

The competition documents the curve exactly:

    price(inv) = base + sign * amp * f(|inv - MKT_I0|)
    sign = +1 if inv < MKT_I0 (scarcity), -1 if inv > MKT_I0 (glut)
    amp  = target * base / f(T)
...
def price_at(product, inventory):
    ...
    if product not in MKT_PARAMS:
        return 0
    base, t, below_func, below_target, above_func, above_target = MKT_PARAMS[product]
    gap = abs(inventory - MKT_I0)
    if gap == 0:
        return base

    scarce = inventory < MKT_I0
...
def crop_value(crop, inventory, pending, days_left=None):
    ...
    if days_left is not None and days_left < days:
        ...
        return -seed_cost
    revenue = revenue_for(crop, inventory.get(crop, MKT_I0) + pending, yield_units)
